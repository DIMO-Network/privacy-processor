...
APPS = privacy-processor
all: $(APPS)

install: $(APPS)
	@mkdir -p bin
	@cp $(PATHINSTBIN)/privacy-processor ./bin/
...
docker: deps
	@docker build -f ./resources/docker/Dockerfile . -t dimozone/privacy-processor:$(VER_CUT)
	@docker tag dimozone/privacy-processor:$(VER_CUT) dimozone/privacy-processor:latest

docker-cgo: deps
	@docker build -f ./resources/docker/Dockerfile.cgo . -t dimozone/privacy-processor:$(VER_CUT)-cgo
	@docker tag dimozone/privacy-processor:$(VER_CUT)-cgo dimozone/privacy-processor:latest-cgo
...
test: $(APPS)
	...
	@$(PATHINSTBIN)/privacy-processor test ./config/test/...
